...
function buildTaskHTML(title) {
  return `
    ...
    <div class="task-action flex gap-3">
      <button class="edit text-xl cursor-pointer">
        <i class="fa-solid fa-pen-to-square"></i>
      </button>
      <button class="delete text-xl cursor-pointer">
...
function handleTaskActions(e) {
  ...
  if (e.target.closest(".edit")) {
    ...
    form.addEventListener("submit", (e) => {
      ...
      if (!newTitle) {
        modal.openModal("Task title cannot be empty.");
        return;
      }
      ...
      if (isDuplicateTask(newTitle, currentIndex)) {
        modal.openModal("Task is available.");
        return;
      }
...
function addTask(e) {
  ...
  if (!raw) {
    modal.openModal("Please enter a task before adding.");
    return;
  }

  if (isDuplicateTask(raw)) {
    modal.openModal("You’ve already added this task.");
    return;
  }
